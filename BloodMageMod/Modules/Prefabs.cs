...
namespace BloodMageMod.Modules
{
    public class Prefabs
    {
        ...
        {
            BloodMagePlugin.DestroyImmediate(main.transform.Find("ModelBase").gameObject);
            BloodMagePlugin.DestroyImmediate(main.transform.Find("CameraPivot").gameObject);
            BloodMagePlugin.DestroyImmediate(main.transform.Find("AimOrigin").gameObject);

            if (Modules.Assets.mainAssetBundle.LoadAsset<GameObject>(modelName) == null)
            {
                Debug.LogError("Trying to load a null model " + modelName);
                return null;
            }
        ...
        private static void SetupCameraTargetParams(GameObject prefab)
        {
            ...
            cameraTargetParams.cameraParams = Resources.Load<GameObject>("Prefab/CharacterBodies/MercBody").GetComponent<CameraTargetParams>().cameraParams;
        ...
        private static void SetupRagdoll(GameObject model) 
        {
            ...
            if (ragdollMaterial == null)
                ragdollMaterial = Resources.Load<GameObject>("Prefabs/CharacterBodies/CommandoBody")
